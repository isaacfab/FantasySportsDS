...
```{r}
data.inel<-FantasySportsDS::ineligible()
...
```{r}
data.gdc<-FantasySportsDS::gamedaycall()
...
for(i in 1:length(DKSalariesExample[,1])){
    ...
    if(!is.na(j)){
      DKSalariesExample$injury[i]<-data.inel[["Injury Type"]][j]
    }
}

for(i in 1:length(DKSalariesExample[,1])){
    j<-stringdist::amatch(DKSalariesExample$DescriptiveName[i],data.gdc$DescriptiveName,maxDist=5)
    if(!is.na(j)){
      DKSalariesExample$gdc[i]<-DKSalariesExample.gdc[["Status"]][j]
    }
}
...
```{r}
forecast<-FantasySportsDS::CombinedForecasts(1,2016)
...
espn_opt<-FantasySportsDS::DKoptLineUpNFL(DKSalariesExample,"ESPN_f")
print(espn_opt[,1:4])

#Optimal Lineup with Average forecast but lower salary cap
avg_opt<-FantasySportsDS::DKoptLineUpNFL(data,"Average_Robust",45000)
print(avg_opt[,1:4])
```
...
hist<-FantasySportsDS::historyESPN(5,2014)

print(hist[,1:4])

#historical performance for week 8 of the 1997 season
hist<-FantasySportsDS::historyNFL(8,1997)
